...
install_warp_cli() {
    ...
    [[ $SYSTEM == "Debian" ]] && [[ ! ${OSID} =~ 9|10|11|12|13 ]] && yellow "当前系统版本：${CMD} \nWARP-Cli代理模式仅支持 Debian 9-13 系统" && exit 1
    [[ $SYSTEM == "Fedora" ]] && yellow "当前系统版本：${CMD} \nWARP-Cli暂时不支持 Fedora 系统" && exit 1
    [[ $SYSTEM == "Ubuntu" ]] && [[ ! ${OSID} =~ 16|18|20|22|24 ]] && yellow "当前系统版本：${CMD} \nWARP-Cli代理模式仅支持Ubuntu 16.04/18.04/20.04/22.04/24.04 系统" && exit 1
